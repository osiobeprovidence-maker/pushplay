        ...
        {campaigns.map((c,i)=> (
          <div key={i} className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-neutral-800 flex items-center justify-center"><Megaphone className="w-6 h-6 text-white" /></div>
              <div><div className="font-bold text-white">{c.title}</div><div className="text-xs text-neutral-500">{c.creator}</div></div>
            </div>
            <div className="flex items-center gap-6">
              ...
              <Button variant="secondary" size="sm" onClick={() => alert('Review campaign - demo')}>Review</Button>
            </div>
          </div>
        ))}
      </div>
